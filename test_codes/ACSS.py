...
class ACSS_App:
    ...
    def setup_model_and_camera(self):
        ...
        try:
            ...
            self.log_action("PiCamera configured")
        except Exception as e:
            print(f"Error initializing PiCamera: {e}. Using placeholder mode.")
            self.picam2 = None
            self.log_action(f"Error initializing PiCamera: {e}")
    ...
    def start_detection(self):
        ...
        try:
            self.picam2.start()
            self.log_action("Camera started")
        except Exception as e:
            print(f"Error starting camera: {e}")
            self.status_label.config(text=f"Error: Camera failed")
            self.log_action(f"Error starting camera: {e}")
            return
    ...
    def stop_detection(self):
        ...
        if self.picam2:
            try:
                ...
                self.log_action(f"Error stopping camera: {e}")
