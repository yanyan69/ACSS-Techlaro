#!/usr/bin/env python3
"""
Automated Copra Segregation System (ACSS) for Raspberry Pi 5.
- Integrates with Arduino via serial for hardware control: servo sorting, motor, IR proximity, AS7263 NIR sensor for moisture.
- Uses Picamera2 for camera preview and capture.
- YOLO for visual classification of copra.
- When IR detects object, triggers YOLO detection, requests AS for moisture, sorts via servo based on class.
- Stores batch statistics in JSON.
"""
...
import sys, threading  # Added for threading and serial
...
CAM_PREVIEW_SIZE = (512, 300)  # Updated to match code's camera_size
# ----------------------------------------------------
...
last_log_time = None
serial_conn = None  # Renamed from serial to avoid conflict
...
latest_frame = None  # For threaded camera preview
...
# Sorting direction mapping based on class
sort_map = {
    "Raw Copra": 'L',
    "Standard Copra": 'C',
    "Overcooked Copra": 'R'
...
def start_sorting(new_batch=True):
    global sorting_active, sorting_start_time, sorting_data, camera_on, picam, batch_id, last_log_time, ir_monitoring, camera_view_on
    if serial is None:
        logging.error("pyserial not available. Cannot start sorting.")
        return
    if new_batch:
        batch_id += 1
        sorting_data = []
    ...
    if not camera_on:
        ...
        if PICAMERA2_AVAILABLE:
            ...
            logging.error("picamera2 not available on this system. Camera disabled.")
    update_right_frame()
...
def stop_sorting():
    global sorting_active, sorting_end_time, camera_on, picam, ir_monitoring, camera_view_on
    sorting_active = False
    ir_monitoring = False
    ...
    send_cmd("MOTOR,OFF")
    close_serial()
    if camera_on and PICAMERA2_AVAILABLE:
        camera_on = False
        if picam:
            picam.stop()
            picam.close()
...
def camera_loop():
    if not PICAMERA2_AVAILABLE:
        return
    global latest_frame, object_detected, waiting_for_as, current_class, current_conf, current_bbox
    frame_counter = 0
    while camera_on and camera_view_on and running:
        try:
            frame = picam.capture_array("main")  # RGB888
            frame_rgb = frame.copy()  # Already RGB

            # Check for object detection trigger from IR
            if object_detected:
                # Run YOLO inference
                results = model(frame_rgb, verbose=False, imgsz=640, conf=0.25, max_det=1)  # Throttled params
                detections = results[0].boxes
                if len(detections) > 0:
                    det = detections[0]  # Take the best detection
                    ...
                    if conf > 0.5:
                        ...
                        cv2.putText(frame_rgb, label, (xmin, max(ymin - 10, 10)),
                                    cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255,255,255), 2)
                        # Request AS for moisture
                        send_cmd("REQ_AS")
                        waiting_for_as = True
                object_detected = False
...
def update_camera_display():
    if camera_view_on and camera_on:
        with frame_lock:
            if latest_frame is not None:
                display_frame = cv2.resize(latest_frame, CAM_PREVIEW_SIZE)
...
def exit_prompt():
    ...
    button_frame.pack(pady=10)
    yes_btn = tk.Button(button_frame, text="Yes", command=lambda: [exit_window.destroy(), on_close()], font=(custom_font.cget("family"), 10), width=8, height=2)
    yes_btn.pack(side='left', padx=5, pady=5, ipadx=10, ipady=5)
...
def on_close():
    global running, camera_on, camera_view_on
    running = False
    camera_on = False
    camera_view_on = False
    time.sleep(0.2)
...
def open_serial():
    ...
    if serial_conn and serial_conn.is_open:
        logging.debug("Serial already open.")
        return
    try:
        serial_conn = serial.Serial(SERIAL_PORT, SERIAL_BAUD, timeout=0.1)
        logging.debug(f"Opened serial {SERIAL_PORT}@{SERIAL_BAUD}")
...
def close_serial():
    ...
    try:
        ...
        logging.error(f"Error closing serial: {e}")

def send_cmd(text):
    if not serial_conn or not serial_conn.is_open:
        logging.error(f"Serial not open. Cannot send: {text}")
        return
    try:
        with serial_lock:
            serial_conn.write((text + "\n").encode())
            logging.debug(f"TX -> {text} (sent successfully)")
    except Exception as e:
        logging.error(f"Serial write failed: {e}")

def serial_reader_thread():
    global ir_state, last_ir_report_time, object_detected, waiting_for_as
    logging.debug("Serial reader started.")
    while serial_conn and serial_conn.is_open and running:
        try:
            line = serial_conn.readline().decode(errors='ignore').strip()
            if not line:
                if ir_monitoring and ir_state and (time.time() - last_ir_report_time > 1.0):
                    ir_state = False
                    logging.debug("IR: No object detected (timeout)")
                continue

            logging.debug(f"RX <- {line} (received from Arduino)")

            if ir_monitoring and line == "IR":
                last_ir_report_time = time.time()
                if not ir_state:  # state changed to detected
                    ir_state = True
                    object_detected = True
                    logging.debug("IR: Object detected - triggering detection")
            elif line.startswith("AS:"):
                if waiting_for_as:
                    vals = line[3:].split(",")
                    logging.debug(f"AS values: {vals}")
                    # Compute moisture (placeholder: random based on class, since no real formula)
                    if current_class == "Raw Copra":
                        moisture = random.uniform(7.1, 10.0)
                    elif current_class == "Standard Copra":
                        moisture = random.uniform(6.0, 7.0)
                    elif current_class == "Overcooked Copra":
                        moisture = random.uniform(0.0, 5.9)
                    else:
                        moisture = 0.0
                    unit = 1.0
                    description = current_class
                    with data_lock:
                        sorting_data.append({'unit': unit, 'description': description, 'moisture': moisture, 'detected_class': current_class, 'confidence': current_conf})
                    logging.debug(f"Added to sorting_data: {current_class}, moisture: {moisture:.2f}, unit: {unit}")
                    # Sort based on class
                    direction = sort_map.get(current_class, 'C')
                    send_cmd(f"SORT,{direction}")
                    # Update GUI
                    root.after(0, update_right_frame)
                    waiting_for_as = False
            elif line == "ACK":
                logging.debug("ACK from Arduino (action completed)")
            else:
                logging.debug("Serial raw: " + line)

        except Exception as e:
            logging.error(f"Serial reader exception: {e}")
            time.sleep(0.1)
...
placeholder_img = load_image('default_icon.png', size=CAM_PREVIEW_SIZE)
...
camera_view_frame = tk.Frame(camera_container, width=CAM_PREVIEW_SIZE[0], height=CAM_PREVIEW_SIZE[1])
