...
class MoistureSorterGUI:
    ...
    def sensor_sort(self):
        ...
        if not self.as_values:
            self._log_message("no AS7263 response~ ignoring us again? 😭")
            return
    ...
    def start_camera(self):
        try:
            if PICAMERA2_AVAILABLE:
                ...
                if cv2 is None:
                    self._log_message("no camera at all? that's sad 💔")
                    return
            ...
            self._log_message("camera start failed: " + str(e) + "~ oopsie 😭")
    ...
    def camera_detect_and_sort(self):
        self._log_message("capturing frame~ smile for the camera lol 📸😜")
        if self.camera_running and self.latest_frame is not None:
            ...
            if PICAMERA2_AVAILABLE:
                ...
                config = temp_picam.create_preview_configuration(main={"size": CAM_PREVIEW_SIZE})
                temp_picam.configure(config)
                temp_picam.start()
                frame = temp_picam.capture_array()
                frame = cv2.cvtColor(frame, cv2.COLOR_RGB2BGR)
                temp_picam.stop()
            elif cv2 is not None:
                temp_cap = cv2.VideoCapture(0)
                temp_cap.set(3, CAM_PREVIEW_SIZE[0])
                temp_cap.set(4, CAM_PREVIEW_SIZE[1])
                ret, frame = temp_cap.read()
                temp_cap.release()
                if not ret:
                    self._log_message("camera capture failed~ boring 💀")
                    return
            else:
                self._log_message("no camera? what even is this setup lol 😭")
                return
